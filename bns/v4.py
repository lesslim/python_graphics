...
def get_color(i, colorl):
	if colorl[i] == 0:
		colorl[i] = color_rgb(randint(25, 255), randint(25, 255), randint(25, 255))
	return colorl[i]

def move_ants(ants, tact, nodes, win, colorl):
	flag = True
	dx = {}
	dy = {}
	for move in tact:
		...
		for move in tact:
			for node in nodes:
		 		if node.name == move[1]:
		 			dest = Point(node.x, node.y)
			ant = ants[int(move[0]) - 1]
			ant['ant'].setFill(get_color(int(move[0]) - 1, colorl))
...
def main():
	...
	if ants_count:
		...
		for tact in ants_tacts:
			move_ants(ants, tact, nodes, win, colorl)
			sleep(.1)
	try:
